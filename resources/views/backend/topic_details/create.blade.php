...
@section('content')
                                ...
                                <form action="{{ route('top_de.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <select id="topic_type" name="topices_slug"
                                                class="form-control select2 select2-info"
                                                data-dropdown-css-class="select2-info" style="width: 100%;">
                                                <option value="" selected="" disabled="">Select Topics Type</option>
                                                @foreach ($topices_type as $row)
                                                    <option value="{{ $row->topics_slug }}">{{ $row->topics_type }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <select name="topic_id" id="topic" class="form-control select2 select2-info"
                                                data-dropdown-css-class="select2-info" style="width: 100%;">
                                                {{-- @foreach ($topices as $row)
                                                    <option value="{{ $row->topices_title_slug }}">
                                                        {{ $row->topices_title }}</option>
                                                @endforeach --}}
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="textarea" id="summernote" name="description" rows="6"
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <input type="file" class="form-control" value="{{ old('video_path') }}"
                                                name="video_path" placeholder="Enter Video path:">
                                            @error('video_path')
                                                <strong class="text-danger">{{ $message }} </strong>
                                            @enderror
                                        </div>
    ...
    <script>
        $(document).on('change', '#topic_type', function() {

            var topic_type = $(this).val();
            $.ajax({
                url: "/topices/get-topics",
                data: {
                    topic_type: topic_type
                },
                success: function(data) {

                    var html = '<option value="" selected="" disabled="">Select Topics</option>'
                    $.each(data, function(key, v) {
                        html += '<option value="' + v.topices_title_slug + '">' + v
                            .topices_title +
                            '</option>'
                    })
                    $('#topic').html(html)
